...
const TOKEN_KEY = 'dac_wf_token'
const API_BASE = import.meta.env.DEV ? '${API_BASE}' : 'https://api.webflow.com'
...
async function uploadAsset(file: File, token: string): Promise<{ fileId: string; url: string }> {
  const hash = await computeMD5(file)
  const initRes = await fetch(`${API_BASE}/v2/sites/${SITE_ID}/assets`, {
    method: 'POST',
  ...
  if (!initRes.ok) {
    ...
    throw new Error(err.message ?? `Asset upload init failed (${initRes.status})`)
  }
...
export default function App() {
  ...
  const setPair = useCallback((id: number, key: 'label' | 'value', value: string) => {
    setForm(prev => ({
      ...prev,
      'project-team': prev['project-team'].map(p => p.id === id ? { ...p, [key]: value } : p),
    }))
  }, [])
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const [mainImageAsset, galleryAssets, projectFileAsset] = await Promise.all([
        files.mainImage ? uploadAsset(files.mainImage, token) : Promise.resolve(null),
        Promise.all(files.gallery.map(f => uploadAsset(f, token))),
        files.projectFile ? uploadAsset(files.projectFile, token) : Promise.resolve(null),
      ])
      ...
      const res = await fetch(
        `${API_BASE}/v2/collections/${COLLECTION_ID}/items`,
        {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
      ...
      if (res.ok) {
        setStatus({ type: 'success', message: `"${data.fieldData.name}" created as draft` })
        setForm(EMPTY)
      ...
      setStatus({ type: 'error', message: err instanceof Error ? err.message : 'Request failed — check your token or network.' })
    }
  }
  ...
  if (!token) {
    return (
      <div className="setup">
        ...
        <h2 className="setup-title">Connect to CMS</h2>
        <p className="setup-desc">
          Enter your Webflow API v2 token once — it'll be saved in this browser.
          Generate one at <strong>Workspace Settings → Integrations → API Access</strong> with <em>CMS Items: write</em> scope.
        </p>
        <input
          className="input"
          type="password"
          ...
          autoFocus
        />
        <button className="btn" onClick={saveToken} disabled={!tokenInput.trim()}>
          Save &amp; Continue
        </button>
      </div>
    )
    ...
    <div className="app">
      ...
      <form className="form" onSubmit={submit}>
        ...
        <section className="section">
          ...
          <div className="row">
            <Field label="Client">
              <input className="input" type="text" value={form.client}
                onChange={e => set('client', e.target.value)} />
            </Field>
            <Field label="Status">
              <input className="input" type="text" value={form.status}
          ...
          <div className="row">
            ...
            <Field label="Area">
              <input className="input" type="text" value={form.area}
                placeholder="450 sqm"
        ...
        <section className="section">
          ...
          <Field label="Project Intro">
            <textarea className="input textarea" rows={3}
              ...
              onChange={e => set('project-summary', e.target.value)} />
          </Field>
          ...
          <Field label="Project Details">
            <textarea className="input textarea" rows={3}
              ...
              onChange={e => set('project-details', e.target.value)} />
          </Field>
            ...
            <div className="pairs">
              {form['project-team'].map((pair) => (
                <div key={pair.id} className="pair-row">
                  <input
                    className="input pair-label"
                    type="text"
                    placeholder="Role"
                    value={pair.label}
                    onChange={e => setPair(pair.id, 'label', e.target.value)}
                  />
                  <span className="pair-sep">:</span>
                  <input
                    className="input pair-value"
                    type="text"
                    placeholder="Name"
                    value={pair.value}
                    onChange={e => setPair(pair.id, 'value', e.target.value)}
                  />
                  <button
                    type="button"
          ...
          <Field label="Image Gallery">
            <input className="input input-file" type="file" accept="image/*" multiple
              ...
              onChange={e => {
                ...
                setFiles(prev => ({ ...prev, gallery: [...prev.gallery, ...added] }))
                e.target.value = ''
              }} />
        ...
        <div className="actions">
          ...
          <button className="btn btn-ghost" type="button"
            onClick={() => { setForm(EMPTY); setFiles(EMPTY_FILES); setStatus({ type: 'idle' }); if (galleryInputRef.current) galleryInputRef.current.value = '' }}>
            Clear
          </button>
        </div>
